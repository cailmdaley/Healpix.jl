...
Healpix map `map`.
"""
function ang2pix(map::OrderedMap{T, Ring}, theta, phi) where {T}
    ang2pixRing(map.resolution, Float64(theta), Float64(phi))
end

function ang2pix(map::OrderedMap{T, Nest}, theta, phi) where {T}
...
`ipix`.
"""
function pix2ang(map::OrderedMap{T, Ring}, ipix) where {T}
    pix2angRing(map.resolution, ipix)
end

function pix2ang(map::OrderedMap{T, Nest}, ipix) where {T}
