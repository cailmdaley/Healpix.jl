...
function saveMaskedMap(mm::MaskedMap{T, O}, path) where {T, O}
	nside = mm.resolution.nside
	writedlm(path, [string.([T O]) ; [nside nside] ; [mm.inds mm.pixels] ])
end

function readMaskedMap(path)
	table = readdlm(path)
	T, O = eval.(Meta.parse.(table[1,:]))
	nside = table[2,1]
